...
DATA_CONTRACTS_EXPORT_DIR = Path(__file__).parent.parent.parent.parent.parent / 'DATA_CONTRACTS_EXPORT_DIR'
    ...
    try:
        ...
        if "genders" in contract_data:
            ...
            for gender_list in genders.values():
                if isinstance(gender_list, list):
                    ...
                    gender_keys.extend([
                        g for g in gender_list 
                        if g and g != "NOT_INCLUDED"
                    ])
        ...
        if "conjugations" in contract_data:
                            ...
                            if isinstance(person_group, dict):
                                for form in person_group.values():
                                    ...
                                    if isinstance(form, str):
                                        ...
                                        cleaned_forms = [
                                            f.strip() for f in form.replace('[', '').replace(']', '').split()
                                        ...
                                        cleaned_forms = [
                                            f for f in form 
                                            if not isinstance(f, str) or (not f.startswith('[') and not f.endswith(']'))
                                        ]
                                        conj_forms.update(cleaned_forms)
            ...
            # If conjugations is a string, split it.
                ...
                # Remove square brackets and split.
                cleaned_forms = [
                    f.strip() for f in conjugations.replace('[', '').replace(']', '').split()
                    if not f.startswith('[') and not f.endswith(']')
                ]
                conj_forms.update(cleaned_forms)
...
def filter_exported_data(
    ...
    try:
        ...
        if data_type == 'nouns':
            columns_to_keep = contract_metadata['nouns']['numbers'] + contract_metadata['nouns']['genders']
...
def export_contracts(input_dir:str = None, output_dir: str = None):
    ...
    for contract_filename in os.listdir(data_contracts):
        ...
        for data_type in data_types:
            # Input file path
            input_file = Path(input_dir) / matched_language.lower().replace(' ', '_') / f"{data_type}.json"
            ...
            # Filter the data
            filtered_data = filter_exported_data(input_file, contract_metadata, data_type)
            ...
            if filtered_data:
                ...
                print(f"Exported {matched_language} {data_type} with {len(filtered_data)} entries")
