...
def check_contract_data_completeness(
    ...
    if language:
        ...
        languages_to_check = [item.name for item in export_dir.iterdir() if item.is_dir()]
    else:
        languages_to_check = [
            Path(f).stem.lower() 
            for f in data_contracts.glob('*.json')
        ]
    ...
    for lang_dir_name in languages_to_check:
        ...
        for data_type in ['nouns', 'verbs']:
            ...
            required_forms = (
                contract_metadata['nouns']['numbers'] + contract_metadata['nouns']['genders'] 
                if data_type == 'nouns' 
            ...
            missing_type_forms = [
                ...
                if not any(form in lexeme_data for lexeme_data in exported_data.values())
            ]
