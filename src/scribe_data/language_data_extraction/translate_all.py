"""
Translates the all words queried from Wikidata to all other Scribe languages.

Example
-------
    python3 src/scribe_data/language_data_extraction/translate_all.py
...
for src_lang in SRC_LANGS:
    print(f"Starting {src_lang} translations...")

    translate_script_dir = Path(__file__).parent
    words_to_translate_path = (
        translate_script_dir / src_lang / "translations" / "words_to_translate.json"
    )
    ...
    translated_words_path = (
        Path(DEFAULT_JSON_EXPORT_DIR) / src_lang / "translations.json"
    )
