...
for l in tqdm(language_word_type_dict, desc="Databases created", unit="dbs",):
    if language_word_type_dict[l] != []:
        ...
        print(f"Fixing {l} database autosuggestions based on nouns data...")

        # Replace autosuggestions with their capitalized equivalents from nouns.
        cursor.execute(
            """
            WITH autosuggestions_update AS (
                SELECT
                    CASE
                        WHEN
                            UPPER(SUBSTR(autosuggestions.suggestion_0, 1, 1)) || SUBSTR(autosuggestions.suggestion_0, 2) = nouns_cap.noun
                        THEN
                            nouns_cap.noun

                        WHEN
                            UPPER(autosuggestions.suggestion_0) = nouns_upper.noun
                        THEN
                            nouns_upper.noun

                        ELSE
                            autosuggestions.suggestion_0
                    END AS suggestion_0,

                    CASE
                        WHEN
                            UPPER(SUBSTR(autosuggestions.suggestion_1, 1, 1)) || SUBSTR(autosuggestions.suggestion_1, 2) = nouns_cap.noun
                        THEN
                            nouns_cap.noun

                        WHEN
                            UPPER(autosuggestions.suggestion_1) = nouns_upper.noun
                        THEN
                            nouns_upper.noun

                        ELSE
                            autosuggestions.suggestion_1
                    END AS suggestion_1,

                    CASE
                        WHEN
                            UPPER(SUBSTR(autosuggestions.suggestion_2, 1, 1)) || SUBSTR(autosuggestions.suggestion_2, 2) = nouns_cap.noun
                        THEN
                            nouns_cap.noun

                        WHEN
                            UPPER(autosuggestions.suggestion_2) = nouns_upper.noun
                        THEN
                            nouns_upper.noun

                        ELSE
                            autosuggestions.suggestion_2
                    END AS suggestion_2,
                ...
                LEFT JOIN
                    nouns AS nouns_cap_0

                ON
                    UPPER(SUBSTR(autosuggestions.suggestion_0, 1, 1)) || SUBSTR(autosuggestions.suggestion_0, 2) = nouns_cap_0.noun

                LEFT JOIN
                    nouns AS nouns_upper_0

                ON
                    UPPER(autosuggestions.suggestion_0) = nouns_upper_0.noun

                LEFT JOIN
                    nouns AS nouns_cap_1

                ON
                    UPPER(SUBSTR(autosuggestions.suggestion_1, 1, 1)) || SUBSTR(autosuggestions.suggestion_1, 2) = nouns_cap_1.noun

                LEFT JOIN
                    nouns AS nouns_upper_1

                ON
                    UPPER(autosuggestions.suggestion_1) = nouns_upper_1.noun

                LEFT JOIN
                    nouns AS nouns_cap_2

                ON
                    UPPER(SUBSTR(autosuggestions.suggestion_2, 1, 1)) || SUBSTR(autosuggestions.suggestion_2, 2) = nouns_cap_2.noun

                LEFT JOIN
                    nouns AS nouns_upper_2

                ON
                    UPPER(autosuggestions.suggestion_2) = nouns_upper_2.noun
            )
            UPDATE
                autosuggestions
            SET
