...
if "Spanish/verbs/" not in file_path:
    with open("verbsQueried.json", encoding="utf-8") as f:
        verbs_list = json.load(f)
else:
    with open("./Spanish/verbs/verbsQueried.json", encoding="utf-8") as f:
        verbs_list = json.load(f)
        update_data_in_use = True

path_from_file = get_path_from_format_file()
path_from_update_data = get_path_from_update_data()
ios_data_dir_from_org = get_ios_data_path("Spanish", "verbs")
android_data_dir_from_org = get_android_data_path("Spanish", "verbs")
desktop_data_dir_from_org = get_desktop_data_path("Spanish", "verbs")
